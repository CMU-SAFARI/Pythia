...
With C<TRUE> optional second argument, it would return an unevaluated array
...
With C<TRUE> optional third argument, would "echo" the commands (preceded by
...
my $matched_par;
$matched_par = qr[[^()]*(?:\((??{$matched_par})\)[^()]*)*];

sub remove_nl ($) { (my $in = shift) =~ s/\n//g; $in }
sub allocatemem_prot ($) {my $mem = shift;  eval {allocatemem $mem; 1} or $@ =~ /^PARI:\s\sat\s+\S*\s+line\s+\d+\.?\s*$/ or die "allocatemem died: $@"}

sub parse_as_gp ($;$$$) {
  my($in, $quote, $echo, $c) = (shift, shift, shift, shift);
  $in =~ s/(\\(?!\\)|\/(?!\*)|[^"\s\\\/]|\n|"([^"\\]|\\.)*")|\/\*.*?\*\/|[^\S\n\/]+|\\\\[^\n]*/ defined($1) ? $1 : '' /ges;
...
#  warn "in: <<$in>>\n";
 ...
 LOOP: {
    for my $IN (@in) {
      print ref $IN ? "? $IN->[2]\n" : "? $IN\n";
      ref $IN ? ($IN->[0] ? $IN->[0]->(PARI($IN->[1])) : last LOOP)
            : (($c and not $IN =~ /;$/) ? (is_gnil($v = PARI($IN)) or print("%$c = $v\n"), $c++ ) : PARI($IN));	# Void context (ignored???)
    }
    print ref $in ? "? $in->[2]\n" : "? $in\n";
    ref $in ? ($in->[0] ? $in->[0]->(PARI($in->[1])) : last LOOP)
            : (($c and not $in =~ /;$/) ? ((is_gnil($v = PARI($in)) or print("%$c = $v\n"), $c++), $v ) : PARI($in))
